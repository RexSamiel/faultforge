...
a `faultforge.reliability.ReliabilityScorer`: `AccuracyScorer`,
`AccuracyDegradationScorer`, `SdcScorer` (Silent Data Corruption - any output
logit changed vs. a golden model), or `Top1SdcScorer` (Critical SDC - the
top-1 prediction changed vs. a golden model). The last three additionally
require a golden (unencoded, or golden-encoded) reference run.
...
`SdcScorer`/`Top1SdcScorer`/`AccuracyDegradationScorer` each additionally run
...
saved.reliability_metric_class()   # the ReliabilityScorer subclass it was recorded with
